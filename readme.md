...
#### Native verified CRX uploads

To create [verified CRX packages](https://developer.chrome.com/docs/webstore/update#protect-package-updates) that protect against package update attacks, you can provide a private key:
...
const store = chromeWebstoreUpload({
  ...
  clientSecret: 'xxxxxxxxxx',
  refreshToken: 'xxxxxxxxxx',
  packExtensionKey: './path/to/privatekey.pem', // optional
});

// When packExtensionKey is provided, only directories are accepted
const response = await store.uploadExisting('./path/to/extension-directory');
// The extension will be packed as a CRX using Chrome's --pack-extension command
```

When `packExtensionKey` is provided:
- Only directories are accepted as input to `uploadExisting()` (not zip or crx files)
- The extension will be packed as a CRX using Chrome's `--pack-extension` command
- The Chrome binary (`google-chrome`) must be available in your PATH
- The private key should be a `.pem` file generated by Chrome or created using the `--pack-extension` command
